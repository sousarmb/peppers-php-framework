...
class IsInteger extends BlackOrWhiteValidator implements Range {
    ...
    public function validate(): bool {
        $this->hasRun = true;
        $filter = filter_var($this->value, FILTER_VALIDATE_INT);
        if ($filter === false) {
            $this->reason = 'Not valid integer value';
        ...
        if (isset($this->_betweenOrNot)) {
            if ($this->_betweenOrNot) {
                ...
                if (($this->_startValue > $this->value) || ($this->value > $this->_endValue)) {
                    $this->reason = 'Value not between validation integers';
                    return $this->isValid = false;
                }
            } else {
                // not between
                if (($this->_startValue <= $this->value) && ($this->value <= $this->_endValue)) {
                    $this->reason = 'Value between validation integers';
                    return $this->isValid = false;
                }
            }
        }
